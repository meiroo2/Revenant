...
public class CheckPoint : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        CheckPointsList = GameObject.FindGameObjectsWithTag("CheckPoint").OrderBy(CP => CP.name).ToList();
    ...
    void SetUpSectionNumber()
    {
        ...
        {
            CheckPointsList[i].GetComponent<CheckPoint>().SectionNumber = i + 1;
        }
    }

    /** 씬 로드 시 체크포인트 활성화 여부 판별 함수 */
    void SetUpIsActivated()
    {
        ...
        {
            // 해당 체크포인트를 활성화
            CheckPointsList[SectionNumber - 1].GetComponent<CheckPoint>().bActivated =
                DataHandleManager.Instance.IsCheckPointActivated;
        }
    }
    ...
    void DestroyAssignedObjects()
    {
        ...
        if (bActivated && SectionNumber <= CheckPointsList[SectionNumber - 1].GetComponent<CheckPoint>().SectionNumber)
        {
            for (int i = 0; i < SectionNumber; i++)
            {
                // 스포너가 있는 체크포인트 일 때 스포너 내부 적 제거
                if (bEnemyListToActivateFromSpawner)
                {
                    DestroyAssignedObjectsList(CheckPointsList[i].GetComponent<CheckPoint>().EnemyListFromSpawner);
                }
                // 등록된 오브젝트 제거 (스포너 등록 가능)
                DestroyAssignedObjectsList(CheckPointsList[i].GetComponent<CheckPoint>().AssignedObjectsList);
            }
        }
    }
    ...
    public static Vector2 GetActiveCheckPointPosition()
    {
        ...
        if (CheckPointsList != null)
        {
            ...
            {
                // 활성화된 체크포인트를 찾아서
                if (CheckPointGameObject.GetComponent<CheckPoint>().bActivated)
                {
                    // 체크포인트 위치를 결과값에 반환
    ...
    public void ActivateCheckPoint()
    {
        ...
        foreach (GameObject CheckPointGameObject in CheckPointsList)
        {
            CheckPointGameObject.GetComponent<CheckPoint>().bActivated = false;
