...
public class LegVisualPart : MonoBehaviour, IPlayerVisualPart
{
    public Sprite[] m_LegSprites = new Sprite[5];
    ...
    public void SetSprite(int _inputIdx)
    {
        ...
        if (_inputIdx >= 0 && _inputIdx < m_LegSprites.Length)
        {
            if (m_CurSpriteIdx != _inputIdx)
            {
                m_CurSpriteIdx = _inputIdx;
                m_SpriteRenderer.sprite = m_LegSprites[m_CurSpriteIdx];
            }
        }
        else
