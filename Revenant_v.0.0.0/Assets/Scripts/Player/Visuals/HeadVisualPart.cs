...
public class HeadVisualPart : MonoBehaviour, IPlayerVisualPart
{
    public Sprite[] m_HeadSprites = new Sprite[5];
    ...
    public void SetSprite(int _inputIdx)
    {
        ...
        if (_inputIdx >= 0 && _inputIdx < m_HeadSprites.Length)
        {
            if (m_CurSpriteIdx != _inputIdx)
            {
                m_CurSpriteIdx = _inputIdx;
                m_SpriteRenderer.sprite = m_HeadSprites[m_CurSpriteIdx];
            }
        }
        else
