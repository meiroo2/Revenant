...
public class JacketVisualPart : MonoBehaviour, IPlayerVisualPart
{
    public Sprite[] m_JacketSprites = new Sprite[5];
    ...
    public void SetSprite(int _inputIdx)
    {
        ...
        if (_inputIdx >= 0 && _inputIdx < m_JacketSprites.Length)
        {
            if (m_CurSpriteIdx != _inputIdx)
            {
                m_CurSpriteIdx = _inputIdx;
                m_SpriteRenderer.sprite = m_JacketSprites[m_CurSpriteIdx];
            }
        }
        else
